                    ...
                    match ctcp_command:
                        case "VERSION" | "version":
                            if tokens.command == "PRIVMSG":
                                await self.send_message(f'NOTICE {sender} :\x01VERSION RudeChat4.2.0\x01')
                                self.add_server_message(f"CTCP: {sender} {target}: {ctcp_command}\n")
                        case "MOO" | "moo":
                            if tokens.command == "PRIVMSG":
                                await self.send_message(f'NOTICE {sender} :\x01MoooOOO! Hi Cow!! RudeChat4.2.0\x01')
                                self.add_server_message(f"CTCP: {sender} {target}: {ctcp_command}\n")
                        case "PING" | "ping":
                            if tokens.command == "PRIVMSG":
                        ...
                        case "FINGER" | "finger":
                            if tokens.command == "PRIVMSG":
                                await self.send_message(f'NOTICE {sender} :\x01FINGER: {self.nickname} {self.server_name} RudeChat4.2.0\x01')
                                self.add_server_message(f"CTCP: {sender} {target}: {ctcp_command}\n")
        ...
        try:
            ...
            if friends_here is not None:
                ...
                if user_info not in self.friends.online_friends:
                    try:
                        self.gui.trigger_desktop_notification(sender=None, usrchan=user_info, message_content="is Online!")
                    except Exception as e:
